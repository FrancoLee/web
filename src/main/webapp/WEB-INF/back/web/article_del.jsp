            ...
            <div class="mt-20">
                <table class="table table-border table-bordered table-bg table-hover table-sort">
                    ...
                    <c:forEach var="art" items="${art_list}">
                        <tr class="text-c">
                            <td class="my-select"><input type="checkbox" value="${art.artId}" name="check"></td>
                            <td>${art.artId}</td>
                            <td class="text-l"><a href="/article?action=showArt&&id=${art.artId}" >${art.title}</a></td>
                            ...
                            <%--<td class="td-status">--%>
                                <%--<c:choose>--%>
                                <%--<c:when test="${art.show==1}">--%>
                                <%--<span class="label label-success radius">--%>
                                    <%--已通过--%>
                                <%--</c:when>--%>
                                <%--<c:when test="${art.show==0}">--%>
                                    <%--<span class="label label-danger radius">--%>
                                    <%--未审核--%>
                                <%--</c:when>--%>
                                <%--<c:when test="${art.show==2}">--%>
                                    <%--<span class="label label-danger radius">--%>
                                    <%--未通过--%>
                                <%--</c:when>--%>
                            <%--</c:choose>--%>
                                <%--</span></td>--%>
                            <td class="f-14 td-manage"><a style="text-decoration:none" onClick="article_rev(this,'${art.artId}')" href="javascript:;" title="还原">还原</a>
                                    <%--<a style="text-decoration:none" class="ml-5" onClick="article_edit('资讯编辑','article-add.html','10001')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>--%>
                                <a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${art.artId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">彻底删除</i></a></td>
                        </tr>
                    </c:forEach>
                    </tbody>
                </table>
...
<script type="text/javascript" src="/back/back_js/article_del.js"></script>
<!--/请在上方写此页面业务相关的脚本-->
</body>
</html>
